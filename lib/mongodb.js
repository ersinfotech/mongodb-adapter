...
MongoDB.prototype.all = function all(model, filter, callback) {
    ...
    var query = this._buildWhere(filter.where);
    ...
    if (filter.limit) {
        cursor.limit(Number(filter.limit));
    }
    if (filter.skip) {
        cursor.skip(Number(filter.skip));
    } else if (filter.offset) {
        cursor.skip(Number(filter.offset));
    }
...
MongoDB.prototype.count = function count(model, callback, where) {
    var query = this._buildWhere(where);
    this.collection(model).count(query, function (err, count) {
        callback(err, count);
    });
};
...
MongoDB.prototype._buildWhere = function (where) {
    var query = {};
    if (where) {
        if (where.id) {
            var id = where.id;
        ...
        Object.keys(where).forEach(function (k) {
            var cond = where[k];
            var spec = false;
            if (cond && cond.constructor.name === 'Array') {
                query[k] = {$in: cond};
                return;
            };
            ...
            if (spec) {
                if (spec === 'between') {
                    ...
                    query[k] = {$regex: new RegExp(cond, 'i')};
                } else {
                    query[k] = {};
                    query[k]['$' + spec] = cond;
                }
